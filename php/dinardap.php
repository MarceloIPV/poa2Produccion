...
if($response === false) {
  ...
  foreach($parser->entidades->entidad->filas->fila->columnas->columna as $msp){ 
    ...
    }else if($msp->campo=="callesDomicilio"){

      $callesDomicilio= $array= explode(":",$msp->valor);

      $jason['callesDomicilio']=$callesDomicilio[0];

    }else if($msp->campo=="domicilio"){

      $domicilio= $array= explode(":",$msp->valor);

      $jason['domicilio']=$domicilio[0];
